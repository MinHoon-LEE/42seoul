...
unsigned long long	calculate(unsigned long long sum, int count, int minus)
{
	if (count >= 20)
	{
		if (minus == - 1)
	...
	{
		if (sum == 922337203685477580678ULL && minus == -1)
			return (-922337203685477580678LL);
		else if (sum > 92233720368547758067ULL)
		{
			if (minus == -1)
				return (0);
			else
...
int		ft_atoi(unsigned char *str)
{
	...
	return (calculate(sum,count,minus));
}
